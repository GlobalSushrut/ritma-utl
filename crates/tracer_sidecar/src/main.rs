...
fn scan_proc_net_egress(index_path: String, namespace_id: String, proc_root: String, interval_secs: u64) -> Result<()> {
    let index = IndexDb::open(&index_path)?;
    let privacy_mode = std::env::var("PRIVACY_MODE").unwrap_or_else(|_| "hash-only".to_string());
    let mut seen: HashSet<String> = HashSet::new();
    loop {
        let tcp_path = format!("{}/net/tcp", proc_root);
                ...
                if cols.len() > 3 {
                    if let Some(dst) = hex_to_ipv4_port(cols[2]) {
                        if seen.insert(dst.clone()) {
                            ...
                            } else { (None, Some(hash_string_sha256(&dst))) };
                            let te = TraceEvent {
                                ...
                                actor: TraceActor { pid: 0, ppid: 0, uid: 0, gid: 0, container_id: None, service: None, build_hash: None },
                                target: TraceTarget { path_hash: None, dst: dst_opt, domain_hash: domain_hash_opt },
...
fn map_line_to_trace(line: &str, namespace_id: &str) -> Option<TraceEvent> {
    ...
    // Defaults
    let mut actor = TraceActor { pid: 0, ppid: 0, uid: 0, gid: 0, container_id: None, service: None, build_hash: None };
    if let Some(pid_s) = parse_token(line, " pid=") { actor.pid = pid_s.parse().unwrap_or(0); }
    if let Some(uid_s) = parse_token(line, " uid=") { actor.uid = uid_s.parse().unwrap_or(0); }
    if let Some(gid_s) = parse_token(line, " gid=") { actor.gid = gid_s.parse().unwrap_or(0); }

    // FILE_OPEN via PATH name= and SYSCALL open/openat
    if (line.contains("type=PATH") || line.contains("name=")) && (line.contains(" open") || line.contains(" openat")) {
        let path_hash = parse_token(line, " name=")
            .map(|s| s.trim_matches('"'))
            .map(|p| hash_string_sha256(p));
        let te = TraceEvent {
            trace_id: format!("te_{}", uuid::Uuid::new_v4()),
            ts: ts.clone(),
            ...
            actor,
            target: TraceTarget { path_hash, dst: None, domain_hash: None },
            attrs: TraceAttrs { argv_hash: None, cwd_hash: None, bytes_out: None },
        };
        return Some(te);
    }
    ...
    if line.contains("type=EXECVE") || (line.contains("type=SYSCALL") && line.contains("syscall=execve")) {
        ...
        let te = TraceEvent {
            ...
            attrs: TraceAttrs { argv_hash, cwd_hash: None, bytes_out: None },
        };
        return Some(te);
    }
...
fn main() -> Result<()> {
    ...
    let net_scan_secs: u64 = std::env::var("NET_SCAN_INTERVAL_SECS").ok().and_then(|s| s.parse().ok()).unwrap_or(30);
    ...
    // Thread 1: auditd tail (fallback)
    {
        ...
        spawn(move || {
            match IndexDb::open(&idx) {
                Ok(index) => {
                    eprintln!("tracer_sidecar[auditd]: tailing {} for {}", ap, ns);
    ...
    // Thread 2: proc net egress scanner
    {
        ...
        spawn(move || {
            eprintln!("tracer_sidecar[proc]: scanning {} every {}s", pr, net_scan_secs);
