...
use index_db::{AttackGraphEdgeRow, IndexDb};
...
impl AttackGraphBuilder {
    ...
    fn compute_graph_hash(&self, edges: &[AttackGraphEdge]) -> String {
        ...
        let mut sorted_edges: Vec<String> = edges
            .iter()
            .map(|e| format!("{}:{}:{}", e.edge_type.as_str(), e.src, e.dst))
            .collect();
        sorted_edges.sort();
    ...
    pub fn compute_velocity(
            ...
            match event.kind {
                TraceEventKind::NetConnect => {
                    if let Some(endpoint) = event
                        ...
                        .dst
                        .as_ref()
                        .or(event.target.domain_hash.as_ref())
                    {
        ...
        let unique_targets_per_minute =
            (unique_targets.len() as f64) / (window_duration_secs / 60.0).max(1.0);
        let escalation_rate = if total_events > 0.0 {
            priv_esc_count as f64 / total_events
        } else {
            0.0
        };
        let lateral_movement_rate =
            (network_targets.len() as f64) / (window_duration_secs / 60.0).max(1.0);
    ...
    pub fn compute_intent(&self, events: &[TraceEvent]) -> IntentAccumulation {
        ...
        let total_intent = (recon_score * 0.2)
            + (access_score * 0.3)
            + (exfil_score * 0.4)
            + (persist_score * 0.1);
    ...
    fn test_graph_building() {
        let events = vec![
            TraceEvent {
                ...
                attrs: TraceAttrs {
                    argv_hash: Some("hash123".to_string()),
                    cwd_hash: None,
                    bytes_out: None,
                    argv: None,
                    cwd: None,
