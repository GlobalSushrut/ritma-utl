use common_models::{IFResult, MLModels, MLScore, NGramResult, WindowRange};
use security_interfaces::{MlRunner, Result};
...
impl MlRunner for SimpleCpuMl {
    fn score_window(
        ...
        let proc_execs = features
            .get("PROC_EXEC")
            .and_then(|v| v.as_u64())
            .unwrap_or(0) as f64;
        let net_new = features
            .get("NET_NEW_DESTS")
            .and_then(|v| v.as_u64())
            .unwrap_or(0) as f64;
        let burst = features
        ...
        let ng_score = if net_new > 0.0 && proc_execs > 0.0 {
            0.4 + (net_new * 0.02).min(0.6)
        } else {
            0.1
        };
