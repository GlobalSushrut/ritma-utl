...
impl CpuPerformanceMonitor {
    ...
    pub fn establish_baseline(&mut self, core_id: u32, counters: CpuPerformanceCounters) {
        log::info!(
            "Establishing baseline for core {}: IPC={:.2}, cache_miss_rate={:.2}%",
            core_id,
            counters.ipc(),
            counters.cache_miss_rate() * 100.0
        );
        self.baselines.insert(core_id, counters);
    }

    /// Monitor for anomalies
    pub fn monitor_counters(
        &mut self,
        counters: CpuPerformanceCounters,
    ) -> Option<HardwareAnomalyAlert> {
        if let Some(baseline) = self.baselines.get(&counters.core_id) {
            // Check cache miss rate
            let cache_miss_deviation = (counters.cache_miss_rate() - baseline.cache_miss_rate())
                .abs()
                / baseline.cache_miss_rate();

            // Check branch miss rate
            let branch_miss_deviation = (counters.branch_miss_rate() - baseline.branch_miss_rate())
                .abs()
                / baseline.branch_miss_rate();

            // Check IPC
            let _ipc_deviation = (counters.ipc() - baseline.ipc()).abs() / baseline.ipc();
            ...
            if branch_miss_deviation > self.threshold {
                ...
                let alert = HardwareAnomalyAlert {
                    ...
                    description: format!(
                        "Branch prediction anomaly on core {} (deviation: {:.2}%)",
                        counters.core_id,
                        branch_miss_deviation * 100.0
                    ),
                ...
                log::error!(
                    "🔴 HARDWARE ANOMALY: Possible Spectre-like attack on core {}",
                    counters.core_id
                );
            ...
            if cache_miss_deviation > self.threshold {
                ...
                let alert = HardwareAnomalyAlert {
                    ...
                    description: format!(
                        "Cache timing anomaly on core {} (deviation: {:.2}%)",
                        counters.core_id,
                        cache_miss_deviation * 100.0
                    ),
                    evidence: HardwareEvidence {
                        counter_values,
                ...
                log::warn!(
                    "🟠 HARDWARE ANOMALY: Possible cache timing attack on core {}",
                    counters.core_id
                );
                self.alerts.push(alert.clone());
                return Some(alert);
            ...
            if let Some(&count) = self.row_access_counts.get(&row) {
                if count > self.rowhammer_threshold {
                    ...
                    let alert = HardwareAnomalyAlert {
                        ...
                        severity: Severity::Critical,
                        description: format!(
                            "Rowhammer attack detected on row 0x{row:x} ({count} accesses)"
                        ),
                        evidence: HardwareEvidence {
                            counter_values,
                    ...
                    log::error!("🔴 ROWHAMMER DETECTED: Row 0x{row:x} accessed {count} times");
...
impl PcieDeviceMonitor {
    ...
    pub fn scan_device(&mut self, mut device: PcieDeviceInfo) -> bool {
        ...
        if suspicious {
            ...
            log::warn!(
                "Suspicious PCIe device detected: {:02x}:{:02x}.{} (vendor: 0x{:04x})",
                device.bus,
                device.device,
                device.function,
                device.vendor_id
