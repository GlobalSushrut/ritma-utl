...
use apt_tracker::{
    AptTracker, AttackStage, BehavioralFingerprint, KillChainStage,
    NetworkConnection as AptNetConn, NetworkPattern, TimingPattern, WindowData,
};
use container_security::{ContainerInfo, ContainerSecurityManager, K8sApiCall};
...
use hardware_monitor::{
    CpuPerformanceCounters, HardwareMonitor, MemoryControllerEvent, MemoryEventType, PcieDeviceInfo,
};
use memory_forensics::{KernelModuleInfo, MemoryForensicsManager, ModuleState};
use ml_detector::{BehavioralFeatureVector, MlDetector};
...
pub fn run_enhanced_demo() {
    ...
    if let Ok(Some(inj)) =
        fdet.detect_process_injection(555, 1337, InjectionType::ProcessVmWritev, None, 128)
    {
    ...
    hard.record_syscall(RawSyscallEvent {
        timestamp_ns: now_ns(),
        pid: 1337,
        tid: 1337,
        uid: 1000,
        ...
        return_value: 0,
        comm: "cryptor".into(),
        entry_point: SyscallEntryPoint::DirectSyscall,
    });
    ...
    if hard.get_direct_syscall_count() > 0 {
        print_alert(
            "T+1ms",
            "CRITICAL",
            "Direct syscall detected (bypassing libc)",
        );
    ...
    if !anomalies.is_empty() {
        print_alert(
            "T+2ms",
            "HIGH",
            &format!("Syscall sequence anomalies: {}", anomalies.len()),
        );
    }
    println!();
    ...
    pset.insert("hash1".to_string());
    pset.insert("hash2".to_string());
    let mut nset = std::collections::HashSet::new();
    nset.insert("evil.com:443".to_string());
    apt.add_window(WindowData {
        window_id: "w1".into(),
        timestamp: 1_000,
        process_hashes: pset.clone(),
        file_hashes: Default::default(),
        network_destinations: nset.clone(),
    ...
    for i in 0..5 {
        apt.record_connection(
            2001,
            AptNetConn {
                timestamp: 10_000 + i * 300,
    ...
    let fp = BehavioralFingerprint {
        process_lineage_pattern: vec!["/usr/bin/curl".into(), "/usr/bin/sh".into()],
        network_pattern: NetworkPattern {
            c2_domains: vec!["evil.com".into()],
            c2_ips: vec![],
            ports: vec![443],
            protocols: vec!["https".into()],
            beaconing_interval_secs: Some(300),
            jitter_percent: Some(5.0),
        },
        timing_pattern: TimingPattern {
            active_hours: vec![1, 2],
            active_days: vec![1],
            sleep_duration_secs: Some(6 * 3600),
            burst_pattern: false,
        },
        syscall_signature: vec![59, 1, 2, 3],
        file_operation_pattern: vec!["/etc/ssl/certs".into()],
        fingerprint_hash: "demo".into(),
    };
    let stage = AttackStage {
        stage: KillChainStage::CommandAndControl,
        timestamp: Utc::now().to_rfc3339(),
        window_id: "w2".into(),
        techniques: vec!["T1071.001".into()],
        indicators: vec!["evil.com:443".into()],
    };
    let _cid = apt
        .get_campaign_attributor()
        .attribute_to_campaign(fp, stage);
    ...
    if !analysis.correlations.is_empty() {
        print_alert(
            ...
            "CRITICAL",
            &format!(
                "APT correlation across {} windows",
                analysis.correlations[0].window_ids.len()
            ),
        );
    ...
    for a in csm.escape_detector().check_escape_attempts("abc123") {
        print_alert("T+75ms", "CRITICAL", &a.description);
    }
    for i in 0..5 {
    ...
    for a in csm.k8s_abuse_detector().analyze_abuse("attacker") {
        print_alert("T+80ms", "CRITICAL", &a.description);
    }
    if let Some(alert) = csm
        .lateral_movement_detector()
        .detect_lateral_movement("pod1", "ns-a", "ns-b")
    {
        print_alert(
            "T+85ms",
            "HIGH",
            &format!(
                "Lateral movement: {} -> {}",
                alert.source_namespace, alert.target_namespace
            ),
        );
    }
    println!();
    ...
    mfm.rootkit_detector()
        .establish_baseline("sys_read".into(), 0xffffffff81000000);
    if let Some(rk) = mfm
        .rootkit_detector()
        .check_syscall_hooks("sys_read", 0xffffffff82000000)
    {
        print_alert("T+100ms", "CRITICAL", &rk.description);
    }
    if let Some(hp) = mfm
        .rootkit_detector()
        .detect_hidden_processes(&[1, 2, 3], &[1, 2, 3, 666, 1337])
    {
        print_alert("T+105ms", "CRITICAL", &hp.description);
    }
    let suspicious = KernelModuleInfo {
        name: "rootkit_module".into(),
        size: 4096,
    ...
    if mfm.kernel_module_analyzer().analyze_module(suspicious) {
        print_alert("T+110ms", "HIGH", "Suspicious kernel module detected");
    }
    let inj = mfm.memory_injection_detector().detect_injection(
        1337,
        "victim".into(),
        0x7fff0000,
        4096,
    ...
    if let Some(dkom) = mfm
        .dkom_detector()
        .detect_process_hiding("malware", true, false)
    {
        print_alert("T+120ms", "CRITICAL", &dkom.description);
    }
    println!();
    ...
    nam.dpi()
        .add_signature("test_malware".into(), vec![0xde, 0xad, 0xbe, 0xef]);
    if let Some(dpi) = nam.dpi().inspect_payload(
        "192.168.1.100".into(),
        "evil.com".into(),
        "TCP".into(),
        &[0x00, 0x01, 0xde, 0xad, 0xbe, 0xef, 0x02],
    ) {
        print_alert("T+150ms", "CRITICAL", &dpi.description);
        print_detail("C2 Pattern", "POST /gate.php (simulated)");
    }
    let efp = nam.fingerprinter().fingerprint_traffic(
        "192.168.1.100".into(),
        "1.2.3.4".into(),
        Some("TLS1.3".into()),
        Some("AES256".into()),
    ...
    if efp.is_suspicious {
        print_alert(
            "T+155ms",
            "HIGH",
            "Encrypted traffic anomaly (unknown JA3, covert timing)",
        );
    }
    println!();
    ...
    hwm.cpu_monitor().establish_baseline(
        0,
    ...
    if let Some(a) = hwm.cpu_monitor().monitor_counters(CpuPerformanceCounters {
        core_id: 0,
        cycles: 1_000_000,
        instructions: 800_000,
        cache_misses: 1_000,
        ...
        print_alert("T+200ms", "CRITICAL", &a.description);
    }
    for _ in 0..1500 {
    ...
    if !hwm.memory_monitor().get_alerts().is_empty() {
        print_alert(
            "T+205ms",
            "CRITICAL",
            "Rowhammer attack detected (1500 row accesses)",
        );
    }
    let suspicious = PcieDeviceInfo {
        bus: 0,
        device: 1,
        function: 0,
        vendor_id: 0xFFFF,
    ...
    if hwm.pcie_monitor().scan_device(suspicious) {
        print_alert(
            "T+210ms",
            "HIGH",
            "Suspicious PCIe device detected (DMA-capable)",
        );
    }
    println!();
    ...
    if let Some(a) = mld.behavioral_detector().detect_anomaly(anomalous) {
        print_alert("T+500ms", "CRITICAL", &a.description);
    }
    ...
    if let Some(r) = mld.threat_hunter().hunt("Living off the Land", &evidence) {
        print_alert(
            "T+500ms",
            "CRITICAL",
            &format!(
                "Automated threat hunting: {} (score {:.2})",
                r.hypothesis, r.threat_score
            ),
        );
    }
    let precursors = vec![
    ...
    if let Some(p) = mld.predictive_engine().predict_threat(&precursors) {
        let horizon = if p.time_to_threat == 0 {
            "not configured".to_string()
        } else {
            format!("{}h", p.time_to_threat.div_ceil(3600))
        };
        print_alert(
            "T+500ms",
            "PREDICT",
            "Predicted: ransomware-like behavior risk",
        );
        print_detail("Horizon", &horizon);
    ...
    println!("{}", "━".repeat(80));
    let (evidence_pack_path, receipt_hash, attack_graph_hash) =
        write_demo_evidence(&ns, &window_id);
...
fn write_demo_evidence(namespace_id: &str, window_id: &str) -> (String, String, String) {
    ...
    let _ = fs::write(
        &evidence_path,
        serde_json::to_string_pretty(&payload).unwrap_or("{}".to_string()),
    );
