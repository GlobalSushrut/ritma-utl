...
impl ProofCircuit {
    ...
    pub fn public_inputs_hash(&self) -> String {
        ...
        let serialized = serde_json::to_string(&self.public_inputs).unwrap_or_default();
...
mod tests {
    use super::*;
    use common_models::{Severity, VerdictExplain, VerdictRangesUsed, VerdictType};
    ...
    fn receipt_chain_circuit_builds() {
        ...
        assert_eq!(
            circuit.public_inputs.get("namespace_id").unwrap(),
            "ns://test/prod/app/svc"
        );
    ...
    fn verdict_attestation_circuit_builds() {
        ...
        assert_eq!(
            circuit.public_inputs.get("verdict_id").unwrap(),
            "verdict_1"
        );
        // VerdictType serializes as snake_case
        assert_eq!(
            circuit.public_inputs.get("verdict_type").unwrap(),
            "policy_violation"
        );
    }
    ...
    fn proof_manager_proves_receipt_chain() {
        ...
        let proof = manager
            .prove_receipt_chain(receipts, "ns://test/prod/app/svc".to_string())
    ...
    fn proof_manager_proves_verdict_attestation() {
        let manager = ProofManager::with_noop_backend();
        let verdict = create_test_verdict();
        let receipts = vec![create_test_receipt("ns://test/prod/app/svc", "genesis")];

        let proof = manager
            .prove_verdict_attestation(verdict, receipts)
