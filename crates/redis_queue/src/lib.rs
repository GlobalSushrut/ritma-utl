...
impl RedisQueue {
    ...
    pub fn pop_ingest<T: for<'de> Deserialize<'de>>(
        &self,
        namespace_id: &str,
    ) -> Result<Option<T>> {
        let mut conn = self.client.get_connection()?;
    ...
    pub fn set_correlation_window<T: Serialize>(
        ...
        let json =
            serde_json::to_string(data).map_err(|e| QueueError::Serialization(e.to_string()))?;
    ...
    pub fn publish_alert<T: Serialize>(&self, namespace_id: &str, alert: &T) -> Result<()> {
        ...
        let json =
            serde_json::to_string(alert).map_err(|e| QueueError::Serialization(e.to_string()))?;
        conn.rpush::<_, _, ()>(key, json)?;
        Ok(())
    }
...
mod tests {
    ...
    fn redis_queue_push_pop() {
        ...
        queue
            .push_ingest("ns://test/prod/app/svc", &event)
            .expect("push");
    ...
    fn redis_queue_correlation_window() {
        ...
        queue
            .set_correlation_window("ns://test/prod/app/svc", "window_1", &data, 60)
            .expect("set");
    ...
    fn redis_queue_dedupe() {
        ...
        let is_dup1 = queue
            .is_duplicate("ns://test/prod/app/svc", "hash_1", 60)
            .expect("check");
        assert!(!is_dup1);
