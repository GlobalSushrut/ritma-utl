use common_models::{
    ArtifactMeta, EvidencePackManifest, PrivacyMeta, SnapshotAction, TraceEvent, TriggerVerdict,
    WindowRange,
};
...
impl Snapshotter {
    ...
    pub fn capture_snapshot(
        ...
        if let Ok(modules) = self.capture_kernel_modules() {
            let (mod_json, mod_redactions) = self
                .privacy
                .redact_secrets(&serde_json::to_string_pretty(&modules).unwrap());
            ...
            for r in mod_redactions {
                redactions.push(r.replacement);
            }
        }

        // 2. Capture process tree
        if let Ok(procs) = self.capture_process_tree() {
            let (proc_json, proc_redactions) = self
                .privacy
                .redact_secrets(&serde_json::to_string_pretty(&procs).unwrap());
        ...
        if let Ok(sockets) = self.capture_sockets() {
            let (sock_json, sock_redactions) = self
                .privacy
                .redact_secrets(&serde_json::to_string_pretty(&sockets).unwrap());
        ...
        if trigger.score >= 0.9 && matches!(trigger.next_action, SnapshotAction::SnapshotStandard) {
            ...
            for event in trace_excerpt {
                if event.actor.uid == 0 && !dumped_pids.contains(&event.actor.pid) {
                    if let Ok(dump_meta) =
                        self.capture_memory_dump(event.actor.pid as i32, "high_severity")
                    {
        ...
        let (trace_json, trace_redactions) = self
            .privacy
            .redact_secrets(&serde_json::to_string_pretty(&trace_excerpt).unwrap());
        let trace_hash = self.hash_content(&trace_json);
        ...
        Ok(EvidencePackManifest {
            ...
            window: WindowRange {
                start: trigger.window.start.clone(),
                end: trigger.window.end.clone(),
            },
    ...
    fn get_interface_stats(&self, iface_name: &str) -> (u64, u64) {
        let rx =
            std::fs::read_to_string(format!("/sys/class/net/{iface_name}/statistics/rx_bytes"))
                .ok()
                .and_then(|s| s.trim().parse().ok())
                .unwrap_or(0);
        let tx =
            std::fs::read_to_string(format!("/sys/class/net/{iface_name}/statistics/tx_bytes"))
                .ok()
                .and_then(|s| s.trim().parse().ok())
                .unwrap_or(0);
        (rx, tx)
                ...
                if let Ok(json_data) = serde_json::from_str::<serde_json::Value>(&stdout) {
                    ...
                    if let Some(items) = json_data["items"].as_array() {
                        for item in items {
                            ...
                            let node_name =
                                item["spec"]["nodeName"].as_str().unwrap_or("").to_string();
                            ...
                            if let Some(container_statuses) =
                                item["status"]["containerStatuses"].as_array()
                            {
                    ...
                    if let Some(items) = json_data["items"].as_array() {
                        for item in items {
                            ...
                            let namespace = item["metadata"]["namespace"]
                                ...
                                .to_string();
                            let cluster_ip =
                                item["spec"]["clusterIP"].as_str().unwrap_or("").to_string();
            ...
            for line in stdout.lines() {
                if line.contains("ESTAB") {
                    ...
                    if parts.len() >= 5 {
                        ...
                        // Extract IPs
                        let client_ip = local_addr
                            .split(':')
                            .next()
                            .unwrap_or("unknown")
                            .to_string();
                        let server_ip = remote_addr
                            .split(':')
                            .next()
                            .unwrap_or("unknown")
                            .to_string();
    ...
    fn capture_http_requests(&self) -> Result<Vec<HttpRequest>, String> {
        ...
        let log_paths = vec!["/var/log/nginx/access.log", "/var/log/apache2/access.log"];
