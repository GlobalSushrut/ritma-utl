    ...
    pub fn verify(&self, manifest: &EvidencePackageManifest) -> VerificationResult {
        ...
        if let Some(ref sig) = manifest.security.signature {
            match self.verify_signature(manifest, sig) {
                ...
                Err(e) => result.errors.push(format!("Signature verification failed: {}", e)),
            }
        } else {
        ...
        match sig.signature_type {
            SignatureType::HmacSha256 => {
                ...
                if let Ok(key_hex) = std::env::var("UTLD_PACKAGE_VERIFY_KEY") {
                    ...
                    let expected = hex::decode(&sig.signature_hex)
            ...
            SignatureType::Ed25519 => {
                ...
                let public_key_hex = sig.public_key_hex.as_ref()
                    .ok_or_else(|| PackageError::SignatureInvalid("missing public key".to_string()))?;
                
                let public_key_bytes = hex::decode(public_key_hex)
                    .map_err(|e| PackageError::SignatureInvalid(format!("invalid pubkey hex: {}", e)))?;
                
                if public_key_bytes.len() != 32 {
                    return Err(PackageError::SignatureInvalid("invalid pubkey length".to_string()));
                }
                ...
                if sig_bytes.len() != 64 {
                    return Err(PackageError::SignatureInvalid("invalid sig length".to_string()));
                }
                ...
                verifying_key.verify(package_hash.as_bytes(), &signature)
                    .map_err(|e| PackageError::SignatureInvalid(format!("ed25519 verify failed: {}", e)))?;
                
                Ok(())
            }
            SignatureType::None => {
                Err(PackageError::SignatureInvalid("no signature type".to_string()))
            }
        }
    }
