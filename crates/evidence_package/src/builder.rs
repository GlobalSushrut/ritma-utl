...
impl PackageBuilder {
    ...
    pub fn build(mut self) -> PackageResult<EvidencePackageManifest> {
        ...
        let package_id = format!("pkg_{tenant}_{now}", tenant = self.tenant_id);
    ...
    fn collect_by_burn(&mut self, burn_id: &str, _framework: &str) -> PackageResult<()> {
        ...
        let burn_path = PathBuf::from(burn_root).join(format!("{burn_id}.json"));

        if !burn_path.exists() {
            return Err(PackageError::MissingArtifact(format!(
                "burn file not found: {burn_path:?}"
            )));
        }
    ...
    fn add_digfile_artifact(&mut self, entry: &DigIndexEntry) -> PackageResult<()> {
        ...
        let path = if let Some(ref p) = entry.storage_path {
            ...
            PathBuf::from(storage_root)
                .join(&entry.root_id)
                .join(format!("{file_id}.json", file_id = &entry.file_id))
        };

        if !path.exists() {
            return Err(PackageError::MissingArtifact(format!(
                "DigFile not found: {path:?}"
            )));
        }
        ...
        let dig: DigFile = serde_json::from_slice(&content).map_err(|e| {
            PackageError::SerializationError(format!("failed to parse DigFile: {e}"))
        })?;
