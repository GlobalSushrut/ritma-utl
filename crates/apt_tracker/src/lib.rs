...
impl CrossWindowCorrelator {
        ...
        for i in 0..window_ids.len() {
            for j in (i + 1)..window_ids.len() {
                ...
                if score > self.threshold {
                    ...
                    log::info!("Correlation found: {} (score: {:.2})", correlation.correlation_id, score);
    ...
    fn compute_correlation_score(&self, w1: &WindowData, w2: &WindowData) -> f64 {
        ...
        let network_overlap = w1.network_destinations.intersection(&w2.network_destinations).count();
        score += (network_overlap as f64) * 0.3;
        
        // Syscall signature similarity
        let syscall_similarity = self.compute_syscall_similarity(&w1.syscall_signature, &w2.syscall_signature);
    ...
    fn find_shared_indicators(&self, w1: &WindowData, w2: &WindowData) -> Vec<String> {
        ...
        for dest in w1.network_destinations.intersection(&w2.network_destinations) {
...
impl DormantBackdoorDetector {
    ...
    pub fn track_process_activity(&mut self, pid: i32, name: String, timestamp: i64) {
        ...
        {
            let info = self.tracked_processes.entry(pid).or_insert_with(|| {
                ProcessTrackingInfo {
                    pid,
                    name: name.clone(),
    ...
    fn detect_dormant_backdoor(&mut self, info: &ProcessTrackingInfo) {
        let avg_sleep = if !info.sleep_periods.is_empty() {
            let total: i64 = info.sleep_periods.iter()
        ...
        let backdoor = DormantBackdoor {
            ...
            severity: if avg_sleep > 3600 { Severity::High } else { Severity::Medium },
        };
        
        log::warn!("Dormant backdoor detected: PID {} (avg sleep: {}s)", info.pid, avg_sleep);
...
impl BeaconPatternRecognizer {
    ...
    pub fn record_connection(&mut self, pid: i32, conn: NetworkConnection) {
        self.connections.entry(pid).or_insert_with(Vec::new).push(conn);
    }
    ...
    pub fn analyze_beaconing(&mut self, pid: i32) -> Option<BeaconPattern> {
        ...
        for conn in conns {
            by_dest.entry(conn.destination.clone()).or_insert_with(Vec::new).push(conn);
        }
        ...
        for (dest, dest_conns) in by_dest {
            ...
            if jitter_percent < 30.0 {  // Less than 30% jitter
                let pattern = BeaconPattern {
                    ...
                    packet_size: dest_conns.iter().map(|c| c.bytes_sent).sum::<usize>() / dest_conns.len(),
                    detected_beacons: dest_conns.len() as u32,
                };
                
                log::warn!("C2 beaconing detected: {} every {}s (jitter: {:.1}%)", 
                          dest, avg_interval, jitter_percent);
...
impl CampaignAttributor {
    ...
    pub fn attribute_to_campaign(
        ...
        for (campaign_id, campaign) in &self.campaigns {
            let similarity = self.compute_fingerprint_similarity(
                &fingerprint,
                &campaign.behavioral_fingerprint,
            );
    ...
    fn compute_fingerprint_similarity(&self, fp1: &BehavioralFingerprint, fp2: &BehavioralFingerprint) -> f64 {
        let mut score = 0.0;
        ...
        let domain_overlap = net1.c2_domains.iter()
...
impl AptTracker {
    ...
    pub fn track_process(&mut self, pid: i32, name: String, timestamp: i64) {
        self.backdoor_detector.track_process_activity(pid, name, timestamp);
    }
    ...
    pub fn analyze(&mut self) -> Result<AptAnalysisResult> {
        ...
        let campaigns = self.campaign_attributor.get_all_campaigns()
            .into_iter()
            .cloned()
            .collect();
...
mod tests {
    ...
    fn test_beacon_pattern_recognition() {
        ...
        for i in 0..5 {
            recognizer.record_connection(1000, NetworkConnection {
