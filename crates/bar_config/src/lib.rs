...
mod tests {
    ...
    fn config_manager_validates_schema() {
        ...
        assert!(manager
            .validate("ns://test/prod/app/svc", &valid_config)
            .is_ok());

        // Invalid config (missing required field)
        let invalid_config = serde_json::json!({"age": 30});
        assert!(manager
            .validate("ns://test/prod/app/svc", &invalid_config)
            .is_err());
    }
    ...
    fn config_manager_merges_layers() {
        ...
        let result = manager
            .load_layered("ns://test/prod/app/svc".to_string(), layers)
            .expect("load layered");
    ...
    fn config_manager_tracks_updates() {
        ...
        // Update and get diff
        let diff = manager
            .update("ns://test/prod/app/svc".to_string(), updated)
            .expect("update");
