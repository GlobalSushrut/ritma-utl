use prometheus::{Counter, Histogram, IntGauge, Registry, Encoder, TextEncoder, HistogramOpts};
...
pub fn init_metrics() {
    REGISTRY.register(Box::new(TRACES_INGESTED.clone())).unwrap();
    REGISTRY.register(Box::new(NOVEL_EGRESS.clone())).unwrap();
    REGISTRY.register(Box::new(AUTH_FAILURES.clone())).unwrap();
    REGISTRY.register(Box::new(WINDOW_PROCESSING_TIME.clone())).unwrap();
    REGISTRY.register(Box::new(ML_SCORE_DISTRIBUTION.clone())).unwrap();
    REGISTRY.register(Box::new(ACTIVE_NAMESPACES.clone())).unwrap();
    REGISTRY.register(Box::new(RECEIPTS_SEALED.clone())).unwrap();
}
