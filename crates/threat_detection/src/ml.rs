...
impl MLAssist {
    ...
    pub fn confidence_delta(&self, event: &DecisionEvent) -> (f64, String) {
        ...
        {
            delta -= 0.08;
            reasons.push("risky_token");
        }
        if alnum_ratio > 0.9 && len <= 16.0 {
            delta += 0.04;
            reasons.push("clean_token");
        }
        if name == "read" || name == "view" || name == "get" {
            delta += 0.04;
            reasons.push("benign_action");
        }

        delta = delta.clamp(-0.1_f64, 0.1_f64);

        let explain = format!("ml_delta={delta:.2} reasons={reasons:?}");
        (delta, explain)
    }
}
...
mod tests {
    ...
    fn make_event(action: &str) -> DecisionEvent {
        DecisionEvent {
            ...
            ts: "2025-12-18T00:00:00Z".into(),
            event_type: action.into(),
            actor: common_models::Actor {
                r#type: common_models::ActorType::User,
                id_hash: "u".into(),
                roles: vec![],
            },
            subject: common_models::Subject {
                r#type: "t".into(),
                id_hash: "s".into(),
            },
            action: common_models::Action {
                name: action.into(),
                params_hash: None,
            },
            context: common_models::Context {
                request_id: None,
                trace_id: None,
                ip_hash: None,
                user_agent_hash: None,
            },
            env_stamp: common_models::EnvStamp {
                env: "dev".into(),
                service: "svc".into(),
                build_hash: "b".into(),
                region: "r".into(),
            ...
            redaction: common_models::RedactionInfo {
                applied: vec![],
                strategy: None,
            },
            stage_trace: vec![],
