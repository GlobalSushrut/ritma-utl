...
impl SequenceDetector {
    pub fn new() -> Self {
        let mut det = Self { patterns: Vec::new(), per_actor: HashMap::new(), max_keep: 20 };
    ...
    fn init_default_patterns(&mut self) {
        ...
        self.patterns.push(AttackSequence {
            ...
            tactics: vec![AttackTactic::Reconnaissance, AttackTactic::InitialAccess, AttackTactic::Execution],
            max_span_secs: 60 * 30, // 30 min window
            severity: 0.8,
        });
        self.patterns.push(AttackSequence {
            name: "Discovery → CredentialAccess → Exfiltration".to_string(),
            tactics: vec![AttackTactic::Discovery, AttackTactic::CredentialAccess, AttackTactic::Exfiltration],
            max_span_secs: 60 * 60, // 1 hour
            severity: 0.8,
        });
        self.patterns.push(AttackSequence {
            name: "PrivilegeEscalation → LateralMovement → Collection → Exfiltration".to_string(),
            tactics: vec![AttackTactic::PrivilegeEscalation, AttackTactic::LateralMovement, AttackTactic::Collection, AttackTactic::Exfiltration],
        ...
        for pat in &self.patterns {
            if let Some(span_ok) = self.tail_matches_in_order(buf, &pat.tactics) {
                if span_ok <= Duration::seconds(pat.max_span_secs) {
                    return Some(SequenceMatch { name: pat.name.clone(), severity: pat.severity });
                }
            }
        }
        None
    ...
    fn tail_matches_in_order(&self, buf: &[(AttackTactic, DateTime<Utc>)], pattern: &[AttackTactic]) -> Option<Duration> {
        ...
        for want in pattern {
            ...
            for i in start..buf.len() {
                if &buf[i].0 == want { found = Some(i); break; }
            }
            if let Some(i) = found { idxs.push(i); start = i + 1; } else { return None; }
        }
