...
impl BehavioralAnomalyDetector {
    ...
    pub fn train(&mut self, normal_samples: Vec<BehavioralFeatureVector>) {
        log::info!(
            "Training behavioral anomaly detector with {} samples",
            normal_samples.len()
        );
    ...
    pub fn detect_anomaly(&mut self, sample: BehavioralFeatureVector) -> Option<MlAnomalyAlert> {
        ...
        if anomaly_score > self.threshold {
            ...
            let alert = MlAnomalyAlert {
                ...
                description: format!("Behavioral anomaly detected (score: {anomaly_score:.2})"),
                features,
                recommended_action: "Investigate process behavior, check for malware".to_string(),
            };

            log::warn!("🟠 ML ANOMALY: Behavioral anomaly detected (score: {anomaly_score:.2})");
...
impl AutomatedThreatHunter {
    pub fn new() -> Self {
        ...
        hunter.hypotheses = vec![
            ...
            ThreatHypothesis {
                ...
                query_patterns: vec!["process_access:lsass.exe".to_string()],
            },
            ThreatHypothesis {
                name: "Data Exfiltration".to_string(),
                description: "Large data transfers to external IPs".to_string(),
                indicators: vec!["large_upload".to_string(), "external_ip".to_string()],
                query_patterns: vec!["network_bytes_sent:>10000000".to_string()],
            },
        ];
    ...
    pub fn hunt(
    ...
    ) -> Option<ThreatHuntingResult> {
        let hypothesis = self.hypotheses.iter().find(|h| h.name == hypothesis_name)?;

        log::debug!(
            "Threat hypothesis loaded: {} ({}); query_patterns={:?}",
            hypothesis.name,
            hypothesis.description,
            hypothesis.query_patterns
        );
        ...
        if !findings.is_empty() {
            ...
            log::warn!(
                "🎯 THREAT HUNT: {} (confidence: {:.2}, threat_score: {:.2})",
                hypothesis.name,
                confidence,
                threat_score
            );
    ...
    pub fn predict_threat(&mut self, observed_precursors: &[String]) -> Option<PredictiveAlert> {
        for pattern in &self.attack_patterns {
            ...
            if match_ratio > 0.6 {
                ...
                let time_to_threat =
                    (pattern.avg_time_to_attack as f64 * (1.0 - match_ratio)) as u64;
                ...
                log::error!(
                    "🔮 PREDICTIVE ALERT: {} predicted in {}s (probability: {:.2})",
                    pattern.name,
                    time_to_threat,
                    probability
                );
