use common_models::{MLScore, Severity, SnapshotAction, TriggerVerdict, Verdict, VerdictType};
...
impl ContractJudge {
    pub fn new(contract: PolicyContract) -> Self {
        Self { contract }
    }
    pub fn with_defaults(namespace_id: &str) -> Self {
        let c = PolicyContract {
    ...
    pub fn judge(&self, ml: &MLScore, features: &serde_json::Value) -> (TriggerVerdict, Verdict) {
        ...
        if !has_violation && ml.final_ml_score >= self.contract.ml_confidence_threshold {
        ...
        let confidence = if has_violation {
            0.9
        } else {
            0.5 + (ml.final_ml_score * self.contract.ml_weight * 0.4)
        };
        let confidence = confidence.min(1.0);

        let next_action = if has_violation && self.contract.snapshot_on_violation {
            if self.contract.block_on_violation {
                SnapshotAction::SnapshotStandard
            } else {
                SnapshotAction::SnapshotMinimal
            }
        } else if ml.final_ml_score >= self.contract.ml_confidence_threshold
            && self.contract.snapshot_on_suspicion
        {
            SnapshotAction::SnapshotMinimal
        } else {
            SnapshotAction::SignalOnly
        };
        ...
        let verdict = Verdict {
            ...
            confidence,
            reason_codes,
            explain: common_models::VerdictExplain {
                summary: Some(ml.explain.clone()),
                evidence_refs: vec![],
            },
            ranges_used: common_models::VerdictRangesUsed {
                json: serde_json::json!({"window": {"start": ml.window.start, "end": ml.window.end}}),
            },
            contract_hash: Some(self.contract.contract_id.clone()),
            policy_pack: None,
        };
