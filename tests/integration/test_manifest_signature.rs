...
fn create_test_proofpack(dir: &std::path::Path) -> std::io::Result<()> {
    ...
    fs::write(dir.join("receipts/public_inputs.json"), serde_json::to_string_pretty(&pub_inputs)?)?;
    
    // Calculate actual hashes
    let manifest_data = fs::read(dir.join("manifest.json"))?;
    let manifest_sha = hex::encode(sha2::Digest::finalize(sha2::Sha256::new().chain_update(&manifest_data)));
    
    let receipts_data = fs::read(dir.join("receipts/public_inputs.json"))?;
    let receipts_sha = hex::encode(sha2::Digest::finalize(sha2::Sha256::new().chain_update(&receipts_data)));
    ...
    fs::write(dir.join("proofpack.json"), serde_json::to_string_pretty(&proofpack)?)?;
...
fn test_signed_proofpack_verifies() {
    ...
    fs::write(dir.path().join("manifest.sig"), serde_json::to_string_pretty(&sig_file).unwrap()).expect("write sig");
...
fn test_tampered_manifest_fails_signature() {
    ...
    fs::write(dir.path().join("manifest.json"), serde_json::to_string_pretty(&manifest).unwrap()).expect("write tampered");
    ...
    let mut proofpack: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(dir.path().join("proofpack.json")).unwrap()
    ).unwrap();
    proofpack["inputs"]["manifest_sha256"] = serde_json::json!(new_manifest_sha);
    fs::write(dir.path().join("proofpack.json"), serde_json::to_string_pretty(&proofpack).unwrap()).expect("write proofpack");
