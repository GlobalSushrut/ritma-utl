...
fn test_canonical_json_key_ordering() {
    ...
    assert_eq!(str1, str2, "canonical JSON must be byte-identical regardless of input key order");
}
...
fn test_export_determinism() {
    ...
    for dir in [dir1.path(), dir2.path()] {
        ...
        fs::write(dir.join("manifest.json"), serde_json::to_string_pretty(&sorted).unwrap()).unwrap();
    }
    ...
    assert_eq!(content1, content2, "canonical JSON exports must be byte-identical");
}
